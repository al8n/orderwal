...
pub(crate) fn construct_inmemory<W: Wal<Ascend, Crc32>>() {
  let mut wal = W::new(WalBuidler::new(Options::new().with_capacity(MB as u32))).unwrap();
  let wal = &mut wal;
  wal.insert(b"key1", b"value1").unwrap();
}

pub(crate) fn construct_map_anon<W: Wal<Ascend, Crc32>>() {
  let mut wal = W::map_anon(WalBuidler::new(Options::new().with_capacity(MB as u32))).unwrap();
  let wal = &mut wal;
  wal.insert(b"key1", b"value1").unwrap();
}
...
pub(crate) fn construct_map_file<W: Wal<Ascend, Crc32>>(prefix: &str) {
  ...
  let path = dir
    .path()
    .join(format!("{prefix}_construct_map_file"));

  {
    let mut wal = W::map_mut(
      &path,
      WalBuidler::new(Options::new()),
      OpenOptions::new()
  ...
  let wal = W::map(&path, WalBuidler::new(Options::new())).unwrap();
  assert_eq!(wal.get(b"key1").unwrap(), b"value1");
}
